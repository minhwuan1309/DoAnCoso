...
import AddForm from "./createBrand";
const Brand = () => {
  ...
  const handleCreate = async (data) => {
    ...
    const res = await fetch(`${BASE_URL}/brands/create`, {
      method: "POST",
      headers: {
  ...
  const handleDelete = async () => {
    const res = await fetch(`${BASE_URL}/brands/delete/${deleteId}`, {
      method: "DELETE",
    });
    setChange(!change);
  ...
  return (
    <Box m="20px">
      <Header title="BRANDS" subtitle="List of Brands" />
      <Button
        variant="contained"
        color="primary"
      ...
      <Dialog open={openCreateDialog}>
        <DialogContent>
          <AddForm createBrand={handleCreate} />
        </DialogContent>
        <DialogActions>
